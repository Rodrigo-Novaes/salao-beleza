...
def init_db():
    ...
    if not c.execute("SELECT 1 FROM servicos LIMIT 1").fetchone():
        c.executemany(
            ...
            ("Corte Feminino","✂️",80,60,"Cabelo","Corte moderno com acabamento perfeito"),
            ("Coloração","🎨",180,120,"Cabelo","Coloração completa com produtos premium"),
            ("Manicure","💅",45,50,"Unhas","Esmaltação com cutilagem e hidratação"),
            ("Sobrancelha Design","🪄",50,40,"Sobrancelha","Design personalizado com henna ou cera"),
            ("Hidratação Capilar","💧",120,90,"Cabelo","Tratamento intensivo para cabelos danificados"),
            ("Maquiagem Social","💄",150,75,"Maquiagem","Make para eventos e ocasiões especiais"),
        ])
        c.executemany(
            "INSERT INTO profissionais (nome,especialidade,telefone,cor,horario_inicio,horario_fim,horario_intervalo) VALUES (?,?,?,?,?,?,?)", [
            ("Ana Clara","Cabeleireira","(11) 99999-1111","#c9a99a","09:00","18:00","12:00"),
            ("Beatriz Lima","Manicure & Pedicure","(11) 99999-2222","#9abecc","08:00","17:00","12:00"),
            ("Carla Mendes","Estética & Sobrancelha","(11) 99999-3333","#a9c99a","10:00","19:00","13:00"),
        ])
        c.executemany("INSERT INTO profissional_servicos VALUES (?,?)",
                      [(1,1),(1,2),(1,5),(2,3),(3,4),(3,6)])
        today = datetime.today().strftime("%Y-%m-%d")
        tom   = (datetime.today()+timedelta(days=1)).strftime("%Y-%m-%d")
        c.executemany(
            "INSERT INTO agendamentos (cliente_nome,cliente_tel,cliente_email,servico_id,profissional_id,data,hora,status,codigo_acesso) VALUES (?,?,?,?,?,?,?,?,?)", [
            ("Maria Silva","(11) 98888-1234","maria@email.com",1,1,today,"10:00","confirmado","MARIA01"),
            ("Paula Souza","(11) 97777-5678","paula@email.com",3,2,today,"14:00","confirmado","PAULA02"),
            ("Fernanda Costa","(11) 96666-9012","fer@email.com",4,3,tom,"11:00","pendente","FERNA03"),
        ])
    conn.commit()
    conn.close()
